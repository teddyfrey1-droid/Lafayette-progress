...
export default function ConnectionControlCenter() {
  ...
  useEffect(() => {
    ...
    // - sinon : uniquement le tenant
    if (!isSuperAdmin && !tenantKey) {
      setLogs([])
      return
    }

    const base = collection(db, "system_logs")
    const q = isSuperAdmin
      ? query(base, orderBy("timestamp", "desc"), limit(500))
      : query(base, where("companyId", "==", tenantKey), orderBy("timestamp", "desc"), limit(500))
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      console.error("system_logs snapshot error", err)
      setLogs([])
    })
