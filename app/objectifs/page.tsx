...
import { objectiveMaxReward, objectiveProgressPct } from "@/lib/bonus-engine"
...
export default function ObjectivesPage() {
  const { objectives, loading, principalMet } = useObjectives()
  ...
  const isPrincipalMet = principalMet;
  ...
  const principalProgress = principalObjective ? objectiveProgressPct(principalObjective) : 0;
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    {!principalObjective.isConfidential && principalObjective.paliers && (
                        <span className="text-xs text-muted-foreground">
                            Max: {(objectiveMaxReward(principalObjective)).toLocaleString()}€
                        </span>
                    )}
                  </div>
                </div>
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-3">
              {secondaryObjectives.map((obj: any) => {
                ...
                let progress = objectiveProgressPct(obj);
