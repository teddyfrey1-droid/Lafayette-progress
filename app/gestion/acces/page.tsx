...
import { authedJsonFetch } from "@/lib/firebase/authed-fetch"
...
export default function GestionDroitsEquipePage() {
  ...
  useEffect(() => {
    ...
    // En contexte multi-tenant, une query trop large échoue avec des règles strictes.
    if (!tenantKey && !currentUser.isSuperAdmin) {
      setUsers([])
      return
    }
    ...
    const recompute = () => {
      ...
      const list = Array.from(merged.values()).filter((u) => u.role !== "super_admin")
      setUsers(list)
    }

    // super_admin sans tenant -> liste globale
    if (currentUser.isSuperAdmin && !tenantKey) {
      const unsub = onSnapshot(query(col), (snapshot) => {
        map1.clear()
        snapshot.docs.forEach((d) => map1.set(d.id, { id: d.id, ...d.data() }))
        recompute()
      })
      return () => unsub()
    }

    const handle = (target: Map<string, any>) => (snapshot: any) => {
      target.clear()
      snapshot.docs.forEach((d: any) => target.set(d.id, { id: d.id, ...d.data() }))
      recompute()
    }

    const unsub1 = onSnapshot(query(col, where("companyId", "==", tenantKey)), handle(map1))
    const unsub2 = onSnapshot(query(col, where("company", "==", tenantKey)), handle(map2))
