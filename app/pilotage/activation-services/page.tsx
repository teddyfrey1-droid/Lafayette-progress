...
function ServiceConfigContent() {
  ...
  const isAllowed = (moduleId: string, roleId: string, mode: "view" | "edit" | "delete") => {
    const perm = permissions[moduleId]
    const list = mode === "edit" ? perm?.edit || [] : mode === "delete" ? (perm as any)?.delete || perm?.edit || [] : perm?.view || []
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Contrôle d'accès par rôle (lecture / modification / suppression). Les changements sont appliqués immédiatement.
            </p>
          </div>
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-muted/50 text-muted-foreground uppercase text-xs">
                <tr>
                  ...
                  {ROLES.map((role) => (
                    <th key={role.id} className="px-4 py-4 text-center font-medium min-w-[140px]">
                      <div className="leading-tight">
                        <div>{role.name}</div>
                        <div className="text-[10px] normal-case opacity-80">V / M / S</div>
                      </div>
                    </th>
                  ))}
                </tr>
                ...
                {MODULES.map((module) => (
                  <tr key={module.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    {ROLES.map((role) => {
                      ...
                      const deleteAllowed = isAllowed(module.id, role.id, "delete")
                        ...
                        <td key={role.id} className="px-4 py-4">
                          <div className="flex items-center justify-center gap-3">
                            ...
                            <div className="flex flex-col items-center gap-1">
                              ...
                              <Switch
                                checked={locked ? true : deleteAllowed}
                                disabled={locked}
                                onCheckedChange={() => updatePermission(module.id, role.id, "delete")}
                                className="data-[state=checked]:bg-primary"
                              />
                            </div>
                          </div>
