...
function renderQuickGroups(){
  ...
  if(groupsArr.length === 0){
    wrap.innerHTML = `<div class="mail-hint">Aucun groupe pour l'instant.</div>`;
    return;
  }
  ...
  wrap.innerHTML = groupsArr.map(g => {
    ...
    return `<div class="mail-chip ${active?'active':''}" onclick="selectMailGroup('${escapeHtml(g.id)}')" title="Sélectionner ce groupe">${escapeHtml(g.name)} (${n})</div>`;
  }).join('');
}
...
function renderUsersGrid(){
  ...
  if(!grid) return;
  const users = getUsersArray().filter(u => u.email && u.email.trim().length > 3).sort((a,b)=> String(a.name).localeCompare(String(b.name)));
  ...
  grid.innerHTML = users.map(u => {
    ...
    return `
      <div class="mail-user-card ${selected?'selected':''}" onclick="toggleMailRecipient('${escapeHtml(u.uid)}')" title="Cliquer pour sélectionner">
      ...
      <div class="user-item" style="border-left:4px solid ${escapeHtml(color)}">
        <div class="user-info">
          <div class="user-header" style="gap:10px">
            ...
            <span class="pub-state on">${ids.length} membres</span>
          </div>
          <div class="user-meta">${escapeHtml(names.join(', '))}${more>0 ? ` +${more}` : ''}</div>
        </div>
...
function renderGroupModalMembers(){
  ...
  members.innerHTML = users.map(u => {
    ...
    return `
      <div class="mail-user-card ${checked?'selected':''}" onclick="toggleMailGroupMember('${escapeHtml(u.uid)}')">
...
async function sendManualEmail(){
  ...
  try{
    ...
    const call = functions.httpsCallable('sendEmailToUsers');
    const res = await call(payload);
    const sent = res?.data?.sent ?? selectedUserIds.size;
    showToast(`✅ Email envoyé à ${sent} personne(s)`);
    
    // Reset
    safeGet('mailSubject').value = '';
    safeGet('mailMessage').value = '';
    clearMailSelection();
    try{ logAction('Mail - Envoi', `${sent} destinataires`); } catch(e){}
  } catch(e){
    console.error(e);
    const msg = (e?.message || '').toLowerCase();
    if(msg.includes('not-found') || msg.includes('functions') || msg.includes('unavailable')){
      alert('La Cloud Function sendEmailToUsers n\'est pas accessible. Vérifie le déploiement et la région.');
    } else {
      alert('Erreur lors de l\'envoi. Vérifi les logs Firebase Functions.');
    }
  }
}
...
async function saveMailGroup(){
  ...
  try{
    ...
    try{ logAction('Mail - Groupe', editingGroupId ? `Update: ${name}` : `Create: ${name}`); } catch(e){}
    closeMailGroupModal();
  } catch(e){
    console.error(e);
