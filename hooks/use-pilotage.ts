...
import { collection, onSnapshot, query, doc, getDoc, updateDoc, arrayUnion, addDoc, orderBy, where } from "firebase/firestore"
...
export function usePilotage() {
  ...
  // MISE À JOUR AVEC DATE PERSONNALISÉE (valeur TOTALE, pas un ajout)
const updateObjectiveProgress = async (objectiveId: string, amount: number, dateObj?: Date) => {
  ...
  if (!Number.isFinite(newTotal)) return false

  try {
    const snap = await getDoc(objRef)
    const prev = snap.exists() ? Number((snap.data() as any)?.current || 0) : 0
