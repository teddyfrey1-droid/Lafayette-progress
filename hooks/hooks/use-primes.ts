  ...
  useEffect(() => {
    ...
    if (isDemo && (profile as any)?.companyId) {
      ...
      const load = () => {
        ...
        const isManager = currentUser?.role === "admin" || currentUser?.role === "gerant"
        const filtered = isManager ? all : all.filter((p: any) => p.userId === currentUser?.uid)
    ...
    const fetchPrimes = async () => {
      ...
      try {
        ...
        if (currentUser?.role === "admin" || currentUser?.role === "gerant") {
          ...
          q = query(
            collection(db, "primes_history"),
            where("userId", "==", currentUser.uid), // <--- ET ICI
            orderBy("date", "desc"),
            limit(20)
          );
        } else {
    ...
    fetchPrimes();
  }, [currentUser, authLoading, isDemo, (profile as any)?.companyId]);
