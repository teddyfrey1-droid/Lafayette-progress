...
function buildTransporter() {
  ...
  if (!host) throw new HttpsError('failed-precondition', 'Missing SMTP_HOST configuration.');
...
exports.sendBulkEmail = onCall(
  ...
  async (request) => {
    ...
    if (!recipients || recipients.length === 0) throw new HttpsError('invalid-argument', 'Recipients required.');
    if (!subject) throw new HttpsError('invalid-argument', 'Subject required.');
    if (!html) throw new HttpsError('invalid-argument', 'Message required.');
    const transporter = buildTransporter();
    const from = `${fromName || process.env.MAIL_FROM_NAME_DEFAULT} <${process.env.MAIL_FROM_EMAIL}>`;
    await transporter.sendMail({ from, bcc: recipients, subject, html });
